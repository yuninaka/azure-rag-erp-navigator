...
**Step 7（CI/CD）に向けた継続検証方針**: `run_ingestion` のような配線部分は、以下の2段構えで
継続的に検証する方針とする。

1. CI（PRごと）: `AzureOpenAI` / `SearchClient` をモックした統合テストを追加し、
   「正しい件数のドキュメントが正しい形で `upload_documents` に渡されるか」を検証する
   （実APIは呼ばない、高速・無料）
2. Staging環境での定期実行: `scripts/verify_azure_connectivity.py` 相当のスモークテストを
   GitHub Actionsのスケジュール実行（例: 日次）でstaging用のAzureリソースに対して実行し、
   実APIとの疎通・スキーマ整合性が壊れていないかを検知する（実APIコストが小さいモデルを使用）

いずれも本Step 2の実装時点では未着手で、Step 7でワークフローとして具体化する。
...
## コードレビューで得られた教訓（2026-09-06）

- 「あり得ないはず」の境界条件（見出しゼロの本文、`overlap_tokens >= max_tokens` 等）は、
  現在のダミーデータでは発生しなくても、将来の別データセットや設定ミスで実際に発生しうる。
  ガード節と回帰テストは、手元の実データの形状に依存せず先回りして用意すべき
  （`chunk_heading_aware` が見出しゼロの本文でチャンクを0件返していた）
- `section_path` のような派生フィールドは、型・存在だけでなく「実際に見出しと対応しているか」
  まで検証しないと、チャンク境界のバグに長期間気づけない
- なお、レビューでは指摘されなかったが実装中に見つけた点として、`chunk_heading_aware` は
  本文中の最初の見出しがオフセット0にない場合、その見出しより前のテキストを暗黙的に
  失う（現在のダミー文書は全てH1が先頭にあるため未顕在化）。別データセット導入時は要確認。
